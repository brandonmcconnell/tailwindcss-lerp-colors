...
describe('lerpColor', () => {
    ...
    describe('invalid option errors', () => {
        ...
        test('should throw error if lerpEnds not boolean', () => {
            expect.assertions(1);
            try {
                (0, index_1.lerpColor)({ 100: '#000' }, {
                    // @ts-expect-error
                    lerpEnds: 'true',
                });
            }
            catch (e) {
                expect(e.message).toBe('tailwind-lerp-colors option `lerpEnds` must be a boolean.');
            }
        });
        // interval
        test('should throw error if interval not integer', () => {
            ...
            try {
                ...
                expect(e.message).toBe('tailwind-lerp-colors option `interval` must be a positive integer greater than 0.');
            }
        });
        // interval negative;
        ...
        test('should throw error if mode not one of validColorModes', () => {
            ...
            try {
                ...
                expect(e.message).toBe(`tailwind-lerp-colors option \`mode\` must be one of the following values: ${index_1.validColorModes.join(', ')}.`);
            }
        });
        ...
        test('should throw error if shades is not valid object', () => {
            ...
            try {
                // @ts-expect-error
                (0, index_1.lerpColor)(null);
            }
            catch (e) {
                expect(e.message).toBe('tailwind-lerp-colors object `shades` must be an object with numeric keys.\n\nvalue used: null');
            }
            try {
                // @ts-expect-error
                (0, index_1.lerpColor)(undefined);
            }
            catch (e) {
        ...
        test('should throw error if shades is object with non-numeric keys', () => {
            ...
            try {
                // @ts-expect-error
                (0, index_1.lerpColor)({ 100: '#000', invalid: '#fff' });
            }
            catch (e) {
                expect(e.message).toBe('tailwind-lerp-colors object `shades` must be an object with numeric keys.\n\nvalue used: {\n  "100": "#000",\n  "invalid": "#fff"\n}');
            }
    ...
    describe('lerpEnds', () => {
        test('should start #ffffff and end #000000 when lerpEnds: true', () => {
            expect.assertions(4);
            const result = (0, index_1.lerpColor)({ 100: '#f1f1f1', 200: '#f3f3f3' }, {
                lerpEnds: true,
            });
        ...
        test('should lerp from #ffffff to start color correctly', () => {
            const result = (0, index_1.lerpColor)({ 100: '#f1f1f1' }, {
                lerpEnds: true,
            });
            expect(result[0]).toBe('#ffffff');
        ...
        test('should lerp from end color to #000000 correctly', () => {
            const result = (0, index_1.lerpColor)({ 900: '#f1f1f1' }, {
                lerpEnds: true,
            });
    ...
    describe('interval boundary conditions', () => {
        ...
        test('should add intervals for input interval < minimum interval from input', () => {
            expect.assertions(2);
            const result1 = (0, index_1.lerpColor)({ 100: '#f1f1f1', 200: '#f3f3f3' }, {
                lerpEnds: false,
                interval: 25,
            });
            expect(Object.keys(result1)).toEqual(['100', '125', '150', '175', '200']);
            // interval 50;
            const result2 = (0, index_1.lerpColor)({ 100: '#f1f1f1', 200: '#f3f3f3' }, {
                lerpEnds: false,
                interval: 50,
            });
        ...
        test('should not add interval between minimum interval, when input interval = minimum interval', () => {
            expect.assertions(1);
            const result = (0, index_1.lerpColor)({ 100: '#f1f1f1', 200: '#f3f3f3', 500: '#f5f5f5' }, {
                lerpEnds: false,
                interval: 100,
            });
    ...
    describe('interval random testing', () => {
        ...
        test(`interval value: 100`, () => {
            const result = (0, index_1.lerpColor)({ 100: '#f1f1f1', 300: '#f3f3f3' }, {
                lerpEnds: false,
                interval: 100,
            });
    ...
    describe('modes', () => {
        index_1.validColorModes.forEach((mode) => {
            // should correctly lerp for mode
            test(`should lerp for mode ${mode}`, () => {
                mockData_1.COLOR_MODES_MOCK_DATA[mode].forEach((data) => {
                    expect((0, index_1.lerpColor)(data.input, { mode, lerpEnds: false })).toEqual(data.output);
                });
            });
        });
    });
    describe('option defaulting', () => {
        ...
        test('should default interval to 25', () => {
            const result = (0, index_1.lerpColor)({ 100: '#f1f1f1', 200: '#f3f3f3' }, { lerpEnds: false });
            expect(Object.keys(result)).toEqual(['100', '125', '150', '175', '200']);
        });
        // should default mode to lrgb;
        test('should default mode to lrgb', () => {
            const result = (0, index_1.lerpColor)({ 100: '#f1f1f1', 200: '#f3f3f3' }, { lerpEnds: false, interval: 50 });
            expect(Object.keys(result)).toEqual(['100', '150', '200']);
        });
        // should default to empty object if options undefined
        test('should default to empty object if options undefined', () => {
            const result = (0, index_1.lerpColor)(mockData_1.COLOR_MODES_MOCK_DATA.lrgb[0].input);
            // does lerping
            expect(result[0]).toBe('#ffffff');
            expect(result[1000]).toBe('#000000');
            expect(result).toMatchObject(mockData_1.COLOR_MODES_MOCK_DATA.lrgb[0].output);
        });
        // should spread empty object when options null;
        test('should spread empty object when options null', () => {
            // @ts-ignore
            const result = (0, index_1.lerpColor)(mockData_1.COLOR_MODES_MOCK_DATA.lrgb[0].input, null);
            // does lerping
            expect(result[0]).toBe('#ffffff');
            expect(result[1000]).toBe('#000000');
            expect(result).toMatchObject(mockData_1.COLOR_MODES_MOCK_DATA.lrgb[0].output);
...
describe('lerpColors', () => {
    ...
    describe('invalid option errors', () => {
        ...
        test('should throw error if includeBase not boolean', () => {
            ...
            try {
                ...
                expect(e.message).toBe('tailwind-lerp-colors option `includeBase` must be a boolean.');
            }
        });
        ...
        test('should throw error if includeLegacy not boolean', () => {
            ...
            try {
                ...
                expect(e.message).toBe('tailwind-lerp-colors option `includeLegacy` must be a boolean.');
            }
        });
        // lerp ends
        test('should throw error if lerpEnds not boolean', () => {
    ...
    const validTailwindColorKeys = Object.keys(colors_1.default);
    ...
    describe('includeBase', () => {
        ...
        test('should include tailwind base colors if includeBase: true', () => {
            const result = (0, index_1.lerpColors)({
                test: {
                    100: '#000',
                },
            }, {
    ...
    describe('includeLegacy', () => {
        ...
        test('should not include tailwind legacy colors if includeLegacy: false', () => {
            ...
            expect(Object.keys(result)).toEqual([...validTailwindColorKeys, 'test'].filter((color) => !legacyColors.includes(color)));
        });
    });
    // lerp ends
    describe('lerpEnds', () => {
        ...
        test('should do lerpEnds for all colors if lerpEnds: true', () => {
            ...
            expect(result.test1).toEqual((0, index_1.lerpColor)({ 100: '#f4f4f4', 200: '#a7a7a7' }, {
                lerpEnds: true,
            }));
            expect(result.test2).toEqual((0, index_1.lerpColor)({
                100: '#f5f5f5',
                200: '#dedede',
            }, {
                lerpEnds: true,
        ...
        test('should not do lerpEnds of all colors if lerpEnds: false', () => {
            ...
            expect(result.test1).toEqual((0, index_1.lerpColor)({
                100: '#f4f4f4',
                200: '#f5f5f5',
            }, {
                lerpEnds: false,
        ...
        describe('should use same interval for all colors', () => {
            [10, 25, 50, 100].forEach((interval) => {
                test(`test interval value: ${interval}`, () => {
                    ...
                    expect(Object.keys(result.test1)).toEqual(Object.keys((0, index_1.lerpColor)({
                        100: '#f4f4f4',
                        200: '#f5f5f5',
                    }, {
                        lerpEnds: false,
                    ...
                    expect(Object.keys(result.test2)).toEqual(Object.keys((0, index_1.lerpColor)({
                        100: '#f5f5f5',
                        300: '#f6f6f6',
                    }, {
                        lerpEnds: false,
    ...
    describe('modes', () => {
        ...
        index_1.validColorModes.forEach((mode) => {
            test(`should use same mode for all colors if mode: ${mode}`, () => {
                const result = (0, index_1.lerpColors)({
                    test1: mockData_1.COLOR_MODES_MOCK_DATA[mode][0].input,
                    test2: mockData_1.COLOR_MODES_MOCK_DATA[mode][1].input,
                    test3: mockData_1.COLOR_MODES_MOCK_DATA[mode][2].input,
                }, {
                    lerpEnds: false,
                    mode,
                    includeBase: false,
                });
                expect(result.test1).toEqual(mockData_1.COLOR_MODES_MOCK_DATA[mode][0].output);
                expect(result.test2).toEqual(mockData_1.COLOR_MODES_MOCK_DATA[mode][1].output);
                expect(result.test3).toEqual(mockData_1.COLOR_MODES_MOCK_DATA[mode][2].output);
            });
        });
    });
    // colors
    ...
    describe('defaulting', () => {
        ...
        test('should default colors to empty object when not passed', () => {
            const result = (0, index_1.lerpColors)(undefined, {
                includeBase: false,
            });
            expect(Object.keys(result)).toEqual([]);
        });
